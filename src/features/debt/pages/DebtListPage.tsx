      ...
      <section className="mx-auto w-full max-w-6xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-semibold tracking-normal">Debt</h1>
        ...
        {!debtsQuery.isLoading && !debtsQuery.isError && (debtsQuery.data ?? []).length === 0 ? (
          <DebtEmptyState />
        ) : null}

        {!debtsQuery.isLoading && !debtsQuery.isError && (debtsQuery.data ?? []).length > 0 ? (
          <DebtList debts={debtsQuery.data ?? []} onDelete={handleDelete} />
        ) : null}
      </section>
    </main>
  );
