    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        {Array.from({ length: 3 }).map((_, index) => (
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm" key={index}>
...
export function DashboardEmptyState() {
  return (
    <div className="rounded-2xl border border-dashed border-border bg-card p-8 text-center">
      ...
      <h2 className="mt-4 font-semibold">Beranda masih kosong</h2>
      <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted-foreground">
        Tambahkan dompet dan catatan uang pertama untuk mulai melihat ringkasan.
      </p>
      <Button asChild className="mt-5 rounded-xl">
        <Link to="/app/wallets">Kelola Dompet</Link>
      </Button>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6 text-card-foreground shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="font-semibold">Beranda gagal dimuat</h2>
